...
@NamedQuery(name = "Product.getProductById",query = "select new com.inn.restaurant.wrapper.ProductWrapper(p.id,p.name,p.description,p.price) from Product p where p.id=:id")
